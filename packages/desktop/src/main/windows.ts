...
const BRAND_NAME = branding.desktop.windowTitle
...
function wireWindowRecovery(win: BrowserWindow, name: string) {
  ...
  win.webContents.on("render-process-gone", (_event, details) => {
    ...
    void show(
      `${BRAND_NAME} window terminated unexpectedly`,
  ...
  win.on("unresponsive", () => {
    ...
    sampler.start()
    void show(`${BRAND_NAME} is not responding`, "You can relaunch the app, open the logs, or keep waiting.", true)
  })
  win.on("responsive", () => {
